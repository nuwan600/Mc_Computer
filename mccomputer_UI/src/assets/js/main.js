angular.module('invoicing', ['angucomplete-alt'])
...
.controller('InvoiceCtrl', ['$scope', '$http', 'DEFAULT_INVOICE', 'DEFAULT_LOGO', 'LocalStorage', 'Currency',
 ...
 $scope.selectedObj = {};
  $http({method : 'GET',url : 'http://localhost:1145/api/mccomputers', headers: { 'X-Parse-Application-Id':'XXXXXXXXXXXXX', 'X-Parse-REST-API-Key':'YYYYYYYYYYYYY'}})
  ...
  $scope.nationalities = [  
     ...
     {  
        "NATIONALITY_ID": 3,
        "description":"TestName3",
        "qty":60,
        "cost":500
     },
     {  
        "NATIONALITY_ID": 4,
        "description":"TestName4",
        "qty":100,
        "cost":5008
     },
     {  
  ...
  $scope.selectedObj = function (selected) {
    if(selected != undefined){
          console.log(selected); 
    $scope.invoice.items.push({ qty:selected.description['qty'], cost:selected.description['cost'], description:selected.description['description'] });
    }
